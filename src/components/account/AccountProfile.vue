    ...
    <div class="account-profile">
      ...
      <div class="account-profile__info">
        ...
        <div class="account-profile__field">
          ...
          <span class="account-profile__value">{{
            userProfile.userCode ?? $t('account.defaultUserCode')
          }}</span>
        </div>
        ...
        <div class="account-profile__field">
          ...
          <span class="account-profile__value">{{
            departmentLabel ?? $t('account.defaultDepartment')
          }}</span>
        </div>
        ...
        <div class="account-profile__field">
          ...
          <span class="account-profile__value">{{
            userProfile.email || userProfile.eposta || 'yunus.senoglu@sompo.com'
          }}</span>
        </div>
        ...
        <div class="account-profile__field">
          ...
          <span class="account-profile__value">{{
            selectedTaskName ?? $t('account.defaultPosition')
          }}</span>
        </div>
...
// Debug kaldırıldı - production'da görünmemesi için

// Birim bilgisi - yeni user object yapısından al
const departmentLabel = computed(() => {
  return (
    userProfile.value?.selectedBirimLabel ||
    userProfile.value?.defaultBirimLabel ||
    accountStore.selectedDepartmentName ||
    $t('account.defaultDepartment')
  )
})

// Görev bilgisi - yeni user object yapısından al
const selectedTaskName = computed(() => {
  return (
    userProfile.value?.selectedGörevLabel ||
    userProfile.value?.defaultGörevLabel ||
    accountStore.selectedTaskName ||
    $t('account.defaultPosition')
  )
})

// Last login time - store'dan al ve formatla
const lastLoginTime = computed(() => {
  const loginDate = userProfile.value.lastLogin ? new Date(userProfile.value.lastLogin) : new Date()
